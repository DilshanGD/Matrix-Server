...
router.post("/admin/class-registration", checkSchema(classValidation), async (req, res) => {
    ...
    try {
        ...
        // Now we need to create records in the ClassBatch table for the batches linked to the class
        const batchPromises = data.batch_id.map(async (batchId) => {
            // Insert a new record into the ClassBatch table for each batch
        ...
        // Wait for all the batch records to be created
        await Promise.all(batchPromises);

        // Respond with the saved class data
